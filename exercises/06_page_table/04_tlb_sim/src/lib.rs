...
impl Mmu {
    ...
    pub fn translate(&mut self, vpn: u64) -> Option<u64> {
        // TODO: 实现 TLB + 页表的二级查找
        //
        if let Some(e) = self.tlb.lookup(vpn, self.current_asid) {
            return Some(e);
        }
        ...
        if let Some((_, pm)) = self
            .page_table
            .iter()
            .find(|&item| item.0 == self.current_asid && item.1.vpn == vpn)
        {
            self.tlb.insert(pm.vpn, pm.ppn, self.current_asid, pm.flags);
            return Some(pm.ppn);
        }
        None
    }
