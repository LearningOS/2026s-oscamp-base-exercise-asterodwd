...
impl Sv39PageTable {
    ...
    pub fn map_page(&mut self, va: u64, pa: u64, flags: u64) {
        ...
        // root ppn is sure to exist
        let mut current_ppn = self.root_ppn;

        let vpns: Vec<_> = (0..3usize).map(|i| Self::extract_vpn(va, i)).collect();

        for level in (1..3usize).rev() {
            let pte = self.nodes.get_mut(&current_ppn).unwrap().entries[vpns[level]];

            if pte & PTE_V == 0 {
                let new_ppn = self.alloc_node();
                let new_pte = (new_ppn << PPN_SHIFT) | PTE_V;

                self.nodes.get_mut(&current_ppn).unwrap().entries[vpns[level]] = new_pte;
                current_ppn = new_ppn;
            } else {
                current_ppn = pte >> PPN_SHIFT;
            }
        }
        self.nodes.get_mut(&current_ppn).unwrap().entries[vpns[0]] =
    ...
    pub fn translate(&self, va: u64) -> TranslateResult {
        ...
        let vpns: Vec<_> = (0..3usize).map(|i| Self::extract_vpn(va, i)).collect();
        let mut offset = va & ((1 << 12) - 1);

        for level in (0..3usize).rev() {
            let pte = self.nodes.get(&current_ppn).unwrap().entries[vpns[level]];
            ...
            if pte & (PTE_R | PTE_W | PTE_X) != 0 {
                offset = va & ((1 << (12 + 9 * (level))) - 1);
                break;
            }
        }

        TranslateResult::Ok(current_ppn * PAGE_SIZE as u64 + offset)
    }
    ...
    pub fn map_superpage(&mut self, va: u64, pa: u64, flags: u64) {
        ...
        self.nodes.get_mut(&current_ppn).unwrap().entries[vpns[1]] =
            (pa >> 12) << PPN_SHIFT | flags;
    }
}
